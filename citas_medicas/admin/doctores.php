...
<div class="main">
    <div class="card">
        ...
        <?php
        // Obtener todos los doctores y su especialidad usando los helpers db_* recién creados
        $doctores = db_fetch_all("SELECT d.id, u.nombre, u.apellido, u.cedula, u.telefono, u.correo, u.genero, u.rol, e.nombre AS especialidad, d.id_especialidad FROM doctores d LEFT JOIN usuarios u ON d.id_usuario = u.id LEFT JOIN especialidades e ON d.id_especialidad = e.id ORDER BY d.id DESC");
        ?>
        <table>
            <tr>
                <th>Nombre</th>
...
function eliminarDoctor(id){
    if(confirm("¿Eliminar este doctor?")){
        ...
        axios.post('ajax/eliminar_doctor.php', formData)
        .then(res=>{
            alert(res.data.message);
            location.reload();
        }).catch(err=>console.error(err));
